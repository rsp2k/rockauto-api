"""Tool information model for individual tools - Static information only."""
...
class ToolInfo(BaseModel):
    """Represents an individual tool with static information (no pricing data)."""
    ...
    features: Optional[str] = Field(None, description="Tool features and capabilities")
...
class ToolWithHistory(BaseModel):
    """Complete tool information including static data and pricing history."""

    # Static tool information (long-term cacheable)
    tool_info: ToolInfo = Field(..., description="Static tool information")
    ...
    def update_pricing(self, price: Optional[str] = None, availability: Optional[str] = None,
        ...
        if self.pricing_history is None:
            self.pricing_history = PriceInfo(part_number=self.tool_info.part_number)
    ...
    def __str__(self) -> str:
        ...
        brand_str = f" ({self.tool_info.brand})" if self.tool_info.brand else ""
        return f"{self.tool_info.name}{price_str}{brand_str}"
